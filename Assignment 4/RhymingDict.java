...
public class RhymingDict {
	...
	public static void storeRhyme(DictionaryInterface rhymingDict, String line) {
		...
		// use getWord() and getRhymeGroup to get word + rhyme group for the line
		String word = getWord(line); // VALUE
		String rhymeGroup = getRhymeGroup(line); // KEY

		// Look up (GET) the key (rhyme group) in the Dictionary (hashtable).
			// if null (no list, create new + add word + put key/value)
		if(rhymingDict.get(rhymeGroup) == null) {

			// Create new MySortedLinkedList, add word to the list
			MySortedLinkedList list = new MySortedLinkedList();
			list.add(word);
			rhymingDict.put(rhymeGroup, list);
		}

		else { // lookup isn't null - already started word list for rhyme group

			//add word to list returned by get() -> linked list
			((MySortedLinkedList)rhymingDict.get(rhymeGroup)).add(word);
		}
	...
	public static void removeUnrhymables(DictionaryInterface rhymingDict) {
		...
		String[] keys = rhymingDict.getKeys();
		int remCount = 0;
		// Iterate thru keys, retrieving value (linked list) w/ each key
		for(int i = 0; i < keys.length; i++) {
			MyLinkedList list = (MyLinkedList)rhymingDict.get(keys[i]);
			
			// if list length = 1, remove
			if(list.size() == 1) {
